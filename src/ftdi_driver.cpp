...
FtdiDriver::~FtdiDriver() {
  ...
  Close();
  if (int_write_buf != NULL) {
    delete [] int_write_buf;
    int_write_buf = NULL;
  }
  if (int_read_buf != NULL) {
    delete [] int_read_buf;
    int_read_buf = NULL;
  }
}
...
#ifdef FT_232R_CBUS0_RXF // an opportunistic way to spot newer library version
    CHECK(FT4222_SPIMaster_SetCS(spi_ft_handle, CS_ACTIVE_LOW));
#else
...
int FtdiDriver::i2c_TransmitX(int sendStartCondition, 
...
int FtdiDriver::i2c_ReceiveX(int sendStartCondition,
                             int sendStopCondition, 
                             int slaveAddress,
                             uint8_t *buf,
                             uint16_t bytesToXfer,
